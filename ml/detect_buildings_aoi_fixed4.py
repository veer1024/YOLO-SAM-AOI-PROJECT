...
def clip_gdf_to_aoi_4326(gdf_4326, bounds_4326, *, buffer_m=0.0):
    """
...
def detect_buildings(bounds):
    ...
    aoi_bounds_4326 = tuple(bounds)  # keep original AOI bounds; don't overwrite later
    ...
    try:
        ...
        # assert isinstance(west, (int, float)) and isinstance(east, (int, float)), \
        #     f"AOI bounds corrupted: {aoi_bounds_4326}"

        # --- Clip final buildings strictly to AOI bounds (robust CRS handling) ---
        # Unpack AOI bounds (EPSG:4326) for clipping/guards
        west, south, east, north = aoi_bounds_4326

        aoi_poly_4326 = shapely_box(west, south, east, north)
        ...
        log.info(f"[AOI GUARD DBG] raster_crs={raster_crs}")
        log.info(f"[AOI GUARD DBG] aoi_poly_raster.bounds={aoi_poly_raster.bounds}")
        # Additional AOI strict containment guard on geometries (before export)
        kept_buildings = []
        if final_buildings:
            first_item = final_buildings[0]
            first_poly = first_item[0] if isinstance(first_item, (tuple, list)) else first_item
            b = first_poly.bounds
            log.info(f"[AOI GUARD DBG] first_building.bounds={b}")
        for item in final_buildings:
            poly = item[0] if isinstance(item, (tuple, list)) else item
            meta = tuple(item[1:]) if isinstance(item, (tuple, list)) else ()
            if poly is None or poly.is_empty:
                continue
            ...
            if meta:
                kept_buildings.append((inter, *meta))
            else:
